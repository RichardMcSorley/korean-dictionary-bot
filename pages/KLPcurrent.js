...
import { START_ADD_PLAYLIST_ITEM, SUCCESS_ADD_PLAYLIST_ITEM, UPDATE_PLAYLIST, SEND_USER_PLAYLIST } from '../webhooks/constants';
    ...
    render() {
        ...
        return (
            <main>
                ...
                {current &&
                              <div>
                    <div className="title"><div><b>{this.shorten(`${current.title} from ${current.owner}`, 55)}</b></div></div>
                ...
                <div className="comming">
                    ...
                    <div>
                    {this.state.playlist.filter((v, i)=>v.videoId !==current.videoId && i <= 1).map((video, index) =>
                        (
                            <div key={index}>
                                    <div><b>{this.shorten(`${index + 1}. ${video.title} from ${video.owner}`, 45)}</b></div>
                                <Divider />
                            </div>
                        )
                    )}
